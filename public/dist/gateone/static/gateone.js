    ...
    var go = GateOne.Base.update(GateOne, {
        ...
        init: function (prefs, /*opt*/callback) {
            ...
            */
            ...
            var authCheck = go.prefs.url + 'auth?check=True&' + 'next=' + encodeURIComponent('/?ssh=' + go.prefs.auth);
            if (go.prefs.auth) {
                // API authentication doesn't need to use the /auth URL.
                logDebug(gettext("Using API authentiation object: ") + go.prefs.auth);

                var url_authenticate_called = localStorage.getItem("url_authenticate_called")
                var url_authenticate_called_time = localStorage.getItem("url_authenticate_called_time")
                var b = new Date().getTime()
                console.log(b - url_authenticate_called_time < 30000)

                if (url_authenticate_called && b - url_authenticate_called_time < 30000) {
                    u.xhrGet(authCheck, parseResponse); // The point of this function is to let the server verify the cookie for us

                } else {
                    var ifr = document.createElement('iframe');
                    ifr.src = go.prefs.url + '?ssh=' + go.prefs.auth;
                    ifr.style = "display:none"
                    ifr.onload = function () {
                        // alert('loaded');
                        localStorage.setItem("url_authenticate_called", 1)
                        localStorage.setItem("url_authenticate_called_time", new Date().getTime())
                        u.xhrGet(authCheck, parseResponse); // The point of this function is to let the server verify the cookie for us
                    };
